...
namespace مشروع_جبر_خطي
{
    public partial class InverseForm : Form
    {
        ...
        private void Transpose()
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.Append(" \n مدور المصفوفه هو تبديل الصفوف مكان الاعمده\n");
            sb.Append("Transpose Of The Matrix : \n");
            sb.AppendLine(new string('-', 50));

            for (int i = 0; i < matrix.TransposeMat.rows; i++)
            {
                for (int j = 0; j < matrix.TransposeMat.cols; j++)
                {
                    sb.Append(matrix.TransposeMat.matrix[i, j].ToString("0.##") + "  \t");
                }
                sb.AppendLine();
            }
            sb.AppendLine(new string('-', 50));
            richTextBoxSolution.Text = sb.ToString();
        ...
        private void MatrixBox_TextChanged(object sender, EventArgs e)
        {
            ...
            btnTranspose.Visible = allFilled;
